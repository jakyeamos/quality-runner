...
| JS app (`BIP-Console`) | `quality-runner refresh /path/to/BIP-Console --run-id-prefix prerelease-dogfood-bip-20260703 --handoff-output /tmp/qr-prerelease-bip-handoff.md --timeout-seconds 60 --verify-timeout-seconds 180 --total-timeout-seconds 300 --total-timeout-reason "pre-release dogfood JS app evidence budget" --json` | blocked | `/tmp/qr-prerelease-bip-handoff.md` | All executable JS gates passed; QR correctly blocked on read-only formatter policy and 10 structural findings. |
| Large messy repo (`BBDSE`) | `quality-runner refresh /path/to/BBDSE --run-id-prefix prerelease-dogfood-bbdse-20260703 --handoff-output /tmp/qr-prerelease-bbdse-handoff.md --timeout-seconds 30 --verify-timeout-seconds 90 --total-timeout-seconds 120 --total-timeout-reason "pre-release dogfood large repo traversal budget" --json` | blocked | `/tmp/qr-prerelease-bbdse-handoff.md` | Timeout evidence now reports total elapsed around 120s, includes phase elapsed, top-level traversal counts, and confirms `.claude/worktrees` is skipped. |
