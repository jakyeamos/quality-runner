...
workflow when a `v*.*.*` tag is pushed. The next release tag is `v0.3.1`.

Do not reuse `v0.1.0`, `v0.2.0`, or `v0.3.0`. `v0.1.0` already reached the
PyPI publish step on 2026-06-28 and failed because the PyPI Trusted Publisher
was not configured for the GitHub OIDC claims. `v0.2.0` was published on
2026-07-02. `v0.3.0` is reserved for the handoff-contract release.
...
7. Verify the published artifact:
   - `curl -sS https://pypi.org/pypi/quality-runner/0.3.1/json`
   - `uv tool install quality-runner==0.3.1 --force`
...
before pushing `v0.3.1`. The release must not be tagged until these claims match
the GitHub workflow.

## Homebrew

Use `packaging/homebrew/quality-runner.rb` as the formula template after the
PyPI source distribution for `0.3.1` is live. Recompute the `sha256` from the
