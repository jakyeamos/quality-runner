...
This guide covers `0.7.0`. `v0.6.0` is the prior published compatibility
baseline. The cutover policy applies when `0.7.0` is installed; until its PyPI
...
```bash
uv tool upgrade 'quality-runner==0.7.0'
```
...
If a published `0.7.0` needs to be rolled back, preserve the run directory and
...
Once `0.7.0` is published, v1 CLI paths that have a direct outcome replacement
(`inspect`, `run`, `verify-gates`, and `review --legacy-output`) are supported
through `0.8.x`. They will not be removed before `0.9.0`, except for a
