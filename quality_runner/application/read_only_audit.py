...
def analyze_read_only_audit(
    ...
    *,
    ...
    if request.cache_mode is None:
        if request.analysis_cache_root is None:
            cache_mode = "disabled"
        elif request.analysis_cache_root == repo_root / ".quality-runner":
            cache_mode = "repo"
        else:
            cache_mode = "external"
    else:
    ...
    if cache_mode == "external" and cache_root is None:
