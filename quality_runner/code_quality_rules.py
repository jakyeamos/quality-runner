...
def _scan_file(
    *,
    ...
    if "debloat" not in disabled_groups and _is_source_file(relative_path):
        is_fat_router = _is_router_path(relative_path) and len(lines) > fat_router_lines
        if len(lines) > large_file_lines and not _is_test_file(relative_path) and not is_fat_router:
            findings.append(
                _finding(
                    category="debloat",
                    severity="warning",
