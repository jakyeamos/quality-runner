...
def save_roadmap(repo_root: Path, roadmap: dict[str, Any]) -> Path:
    ...
    content = _replace_machine_block(path.read_text(encoding="utf-8"), ROADMAP_MARKER, roadmap)
...
def write_verification_file(path: Path, payload: dict[str, Any], body: str) -> Path:
    if path.exists():
        content = _replace_machine_block(
            path.read_text(encoding="utf-8"), VERIFICATION_MARKER, payload
        )
    else:
...
def _write_machine_document(path: Path, marker: str, payload: dict[str, Any], body: str) -> None:
