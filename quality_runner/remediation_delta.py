...
def build_remediation_delta(
    *,
    ...
    return {
        ...
        "slices": {
            "added": [
                current_slices[key] for key in sorted(current_slice_ids - baseline_slice_ids)
            ],
            "persisted": [
                {
                    "id": key,
...
def _package_diff_paths(repo_root: Path, baseline_dir: Path, current_dir: Path) -> list[str]:
    ...
    if result.returncode != 0:
        return []
    return sorted(path for path in result.stdout.splitlines() if path and _is_package_path(path))
