...
def apply_audit_resolutions(
    ...
    *,
    ...
    unresolved = [
        finding for finding in annotated if not _finding_is_resolved(finding.get("resolution"))
    ]
    return {
        **report,
...
def _audit_finding_resolution(
    ...
    *,
    ...
    matched = [
        ...
        if isinstance(entry.get("fingerprint"), str) and entry["fingerprint"] in fingerprints
    ]
    if not matched:
        return {
            "status": "unresolved",
