...
def start_review_loop(*, cycle_id: str, stop_condition: str = "critical-high") -> LoopState:
    ...
    return {
        ...
        "fixing_agent_status": "not-started",
        "active_cycle": True,
    }
...
def select_handoff_findings(
    ...
    *,
    ...
    all_critical_high: bool = False,
) -> list[dict[str, object]]:
    selected = set(finding_ids)
    return [
        dict(finding)
        for finding in findings
        if (finding.get("id") in selected)
        or (all_critical_high and finding.get("severity") in {"critical", "high"})
    ]


def next_iteration(
    state: LoopState, *, selected_finding_ids: Sequence[str], fixing_agent_status: str = "complete"
) -> LoopState:
    ...
    return {
        **state,
        ...
        "selected_finding_ids": list(selected_finding_ids),
        "fixing_agent_status": fixing_agent_status,
    }


def finalize_review_loop(
    state: LoopState, findings: Sequence[Mapping[str, object]]
) -> dict[str, object]:
    return {
        ...
        "stopped": should_stop(findings, state["stop_condition"]),
        "findings": [dict(finding) for finding in findings],
        "uncertainty": "fixing agent changed scope"
        if state["fixing_agent_status"] == "scope-changed"
        else None,
    }
