...
def security_review_handoff(
    ...
    executable_gates = [
        gate
        for gate in _mappings(security_scan.get("available_capabilities"))
        if gate.get("capability_kind") == "local_command"
    ]
...
def security_review_markdown(security_review: object) -> list[str]:
    if not isinstance(security_review, dict):
        return []
    security_review = cast(dict[str, Any], security_review)
    ...
    if isinstance(agent_gates, list) and agent_gates:
        for gate in _mappings(agent_gates):
            gate_id = gate.get("id")
            ...
            if scope is not None:
                ...
                if isinstance(paths, list) and paths:
                    lines.append("Scope:")
                    lines.extend(f"- {path}" for path in _strings(paths))
                    lines.append("")
                if isinstance(categories, list) and categories:
                    lines.append("Review categories:")
                    lines.extend(f"- {category}" for category in _strings(categories))
                    lines.append("")
            instructions = gate.get("review_instructions")
            if isinstance(instructions, list) and instructions:
                lines.append("Instructions:")
                for index, instruction in enumerate(
                    _strings(instructions),
                    start=1,
                ):
            ...
            if isinstance(completion, list) and completion:
                ...
                lines.extend(f"- {item}" for item in _strings(completion))
                lines.append("")
    else:
