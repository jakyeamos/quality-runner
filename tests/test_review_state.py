...
def test_known_issue_accept_edit_remove_and_repeat(tmp_path: Path) -> None:
    ...
    assert (
        known_issue_findings(tmp_path, [{"fingerprint": "fp", "status": "open"}])[0][
            "classification"
        ]
        == "known-accepted"
    )
...
def test_major_change_triggers_cover_declared_inputs() -> None:
    ...
    assert major_change_requires_reverification(
        changed_files=["infra/deploy.yml"], configured_high_risk_paths=["infra"]
    )
    assert major_change_requires_reverification(explicit=True)
    assert not major_change_requires_reverification(
        changed_files=["src/a.py"], configured_high_risk_paths=["infra"]
    )


def test_cycle_finalization_classifies_resolved_and_accepted() -> None:
    state = finalize_cycle_state(
        cycle_id="c1",
        findings=[{"fingerprint": "open"}],
        prior_findings=[{"fingerprint": "gone"}, {"fingerprint": "open", "status": "accepted"}],
    )
